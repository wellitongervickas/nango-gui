...
## Known limitations (non-goals for v1.2)

- **`_index.json` leaks key names** — The namespace index stores key names (e.g. `oauth-app:slack:clientSecret`) in plaintext. Secret *values* are encrypted, but the *presence* of a secret under a given key is observable by anyone with filesystem access to `<userData>/secrets/_index.json`. This is an accepted trade-off for v1.2: mitigating it would require OS-keychain enumeration (keytar's `findCredentials`) which adds a native dependency. File a follow-up if at-rest key-name confidentiality is required.

- **Caller migration deferred** — Existing callers in `credential-store.ts` and `ipc-handlers.ts` still read/write secrets directly via Electron `safeStorage`. Migrating those call sites to route through `SecretStore` is tracked in the F2/F5 follow-on work. Until then, acceptance criterion #3 ("no code path outside `SecretStore` reads/writes secret material") is not fully satisfied.
