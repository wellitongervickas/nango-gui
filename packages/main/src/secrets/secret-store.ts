...
export function getSecret(key: string): string | null {
  ...
  try {
    ...
    log.warn(`[SecretStore] failed to decrypt secret "${key}" — blob may be corrupt: ${String(err)}`);
    return null;
  }
}
...
/** Type alias for consumers who want to type-check against the SecretStore shape. */
export type SecretStoreApi = typeof SecretStore;
